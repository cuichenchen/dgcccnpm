...
module.exports = cmd => {
  if (!argv) {
    argv = program
      ...
      .option('-r, --registry [registry]', 'registry url, default is ' + config.dgcccnpmRegistry)
      .option('-w, --registryweb [registryweb]', 'web url, default is ' + config.dgcccnpmHost)
      ...
      .option('--ignore-custom-config', 'ignore custom .dgcccnpmrc')
  ...
  argv.on('version', function() {
    console.log('dgcccnpm@%s (%s)%snpm@%s (%s)%snode@%s (%s)%snpminstall@%s (%s)%sprefix=%s %s%s %s %s %sregistry=%s',
      pkg.version,
      __filename,
      os.EOL,
  ...
  argv.registryweb = argv.registryweb || config.dgcccnpmHost;
...
function getDefaultRegistry(userconfig) {
  ...
  return config.dgcccnpmRegistry;
}
