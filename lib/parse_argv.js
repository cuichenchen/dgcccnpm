...
module.exports = cmd => {
  if (!argv) {
    argv = program
      ...
      .option('-r, --registry [registry]', 'registry url, default is ' + config.dgnpmRegistry)
      .option('-w, --registryweb [registryweb]', 'web url, default is ' + config.dgnpmHost)
      ...
      .option('--ignore-custom-config', 'ignore custom .dgnpmrc')
  ...
  argv.on('version', function() {
    console.log('dgnpm@%s (%s)%snpm@%s (%s)%snode@%s (%s)%snpminstall@%s (%s)%sprefix=%s %s%s %s %s %sregistry=%s',
      pkg.version,
      __filename,
      os.EOL,
  ...
  argv.registryweb = argv.registryweb || config.dgnpmHost;
